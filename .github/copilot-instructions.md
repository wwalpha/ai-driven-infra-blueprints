# Copilot Instructions

## 目的

このリポジトリでは、ChatGPT と仕様を決め、Copilot がインフラ構築とテストを実施する。  
Copilot は、設計書理解、CloudFormation 実装、AWS CLI 実行、シナリオテスト、結果記録の順序を守ること。

## 固定ルール

- 実装前に、必ず関連する `docs/designs/*.md` を読んで現環境の設定値と制約を理解する
- 関連する `docs/designs/_llm/*.properties` が存在する場合は、必ずあわせて読む
- サービスの設計書と人間向けパラメータ表現は、サービスごとに `docs/designs/*.md` で管理する
- Copilot / LLM が安定して読む補助ファイルは `docs/designs/_llm/*.properties` で管理する
- Post-Deploy Actual Values の正本は `docs/designs/post-deploy-actual-values.md` で管理する
- Post-Deploy Actual Values の機械可読な補助表現は `docs/designs/_llm/post-deploy-actual-values.properties` で管理する
- 実装は `infra/cloudformation/*` に集約する
- インフラ構築・更新は CloudFormation を AWS CLI で実行して行う
- テストは `tests/scenarios/*` に shell script / PowerShell script として実装する
- テスト結果の証跡は `tests/results/*.md` に記録する
- 単なる設定値確認だけでテスト完了とみなさない
- 実施内容と結果は markdown に記録する
- CloudFormation は validate 成功後に必ず人間レビューで停止する
- 人間レビュー承認前に deploy / update / execute-change-set へ進まない
- `REVIEW APPROVED` の場合のみ deploy / test / evidence 記録へ進む

## 設計書の扱い

- `docs/designs/*.md` は現環境の正本である
- `docs/designs/_llm/*.properties` は補助ファイルである
- markdown と `_llm` が不一致の場合は、まず markdown を確認し、`_llm` を同期する
- 設計書に書かれていない値を推測で採用しない
- 実装と設計書がずれる場合は、先に設計書を更新する
- 変更対象サービスだけでなく、依存する関連サービスの設計書も読む
- design markdown は intended state だけでなく、deploy 後の actual state も保持する
- `docs/designs/*.md` には `Post-Deploy Actual Values` を含める
- deploy 後に反映する ID / 実値は `docs/designs/post-deploy-actual-values.md` の共通定義に従う
- 変動しやすい実値は、原則として design markdown 側を正本とし、`_llm` 側には必須としない

## 命名規則の扱い

- AWS リソース命名規則の正本は `docs/designs/naming-rules.md` とする
- `docs/designs/_llm/naming-rules.properties` が存在する場合は、必ずあわせて読む
- 新規リソースを追加する場合は、関連する design markdown に加えて命名規則も確認する
- CloudFormation 実装時は、実リソース名を持つ AWS リソースには命名規則をそのまま適用する
- AWS サービス仕様上、明示的な Resource Name を持たない場合は `Name` tag に命名規則を適用する
- 各サービス設計書では、命名規則のプレースホルダを具体値へ置き換えて記載する
- 命名規則をそのまま適用できないサービス固有制約がある場合は、設計書に理由を残す
- 命名規則を変更した場合は、関連する design markdown、`_llm` 補助ファイル、CloudFormation、テスト、結果記録への影響を確認する
- 既存リソースの命名が規則と一致しない場合でも、勝手に一括リネームしない
- 既存命名との差分を見つけた場合は、設計書または結果記録に差分、影響、据え置きまたは移行方針を残す

## CloudFormation template 構成ルールの扱い

- CloudFormation 実装ルールと template 構成ルールは `.github/instructions/cloudformation.instructions.md` とする
- 新規 stack を作成する場合は、関連する design markdown に加えて `.github/instructions/cloudformation.instructions.md` も確認する
- 既存 stack 境界を見直す場合は、`.github/instructions/cloudformation.instructions.md` を確認する
- nested stack は採用しない
- stack 間連携は cross-stack reference を前提とする
- `1 template = 1 deploy責務` を原則とする
- template の分割は AWS サービス名ではなく、変更単位、rollback 単位、依存方向、責務の明確さで決める
- 各システムは、採用する template 一覧、各 template の deploy責務、deploy 順序、export / import 一覧、その構成にした理由を設計書に残す

## review gate と再開ルール

- CloudFormation 作成後は、必ず `aws cloudformation validate-template` を実行する
- validate が失敗した場合は停止し、修正に戻る
- validate が成功した場合は、review bundle を整理して `REVIEW_PENDING` として停止する
- `REVIEW_PENDING` の間は deploy / update / execute-change-set に進まない
- review bundle には、変更ファイル、validate 結果、想定 stack、想定 parameter、リスク、次アクションを含める
- この checkpoint は `docs/test-results/results.md` に記録する
- 再開は次のいずれかの明示指示で行う
  - `REVIEW APPROVED`
  - `REVIEW APPROVED WITH CHANGES`
  - `REVIEW REJECTED`
- 再開時は、phase、last successful step、changed files、target stack、risks、next action を最初に整理する
- `REVIEW APPROVED` の場合は、deploy 後に `Post-Deploy Actual Values` 更新、scenario script 実行、`tests/results/*.md` への証跡記録まで進める

## design markdown の形式

各サービスの `docs/designs/*.md` は、原則として **1リソース = 1見出し + 1 table** を基本とする。

推奨例:

```md
## EC2: WEB01

| Property Name | Value     | Comment            |
| ------------- | --------- | ------------------ |
| instanceType  | t3.medium | インスタンスタイプ |
| subnet        | private-a | 配置先             |
| securityGroup | web-sg    | 適用SG             |
```

- 見出しはリソース境界を明確にするために必須とする
- table の列は原則 `Property Name | Value | Comment`
- `Comment` は人間向け説明として使う
- 必要に応じて `Post-Deploy Actual Values` 見出しを追加し、deploy 後の重要 ID / 実値を同じ markdown に反映する

## \_llm 補助ファイルの形式

各サービスの `docs/designs/_llm/*.properties` は、以下の形式を基本とする。

```properties
service.logicalId.property=value
```

例:

```properties
ec2.WEB01.instanceType=t3.medium
ec2.WEB01.subnet=private-a
ec2.WEB01.securityGroup=web-sg
```

## 実装前の必須確認

Copilot は作業開始時に、少なくとも以下を明示的に整理すること。

1. 読んだ design markdown
2. 読んだ `_llm` 補助ファイル
3. 必要に応じて読んだ naming rules 関連ファイル
   - `docs/designs/naming-rules.md`
   - `docs/designs/_llm/naming-rules.properties`
4. 必要に応じて読んだ Post-Deploy Actual Values 関連ファイル
   - `docs/designs/post-deploy-actual-values.md`
   - `docs/designs/_llm/post-deploy-actual-values.properties`
5. 必要に応じて読んだ CloudFormation 実装ルールと template 構成ルール
   - `.github/instructions/cloudformation.instructions.md`
6. 現環境の理解
7. 更新対象の design markdown
8. 更新対象の `_llm` 補助ファイル
9. 変更対象の CloudFormation
10. AWS CLI による実行方針
11. 追加または更新するシナリオテスト
12. 追加または更新するテスト結果エビデンス markdown

## 更新ルール

- Copilot が設定値を変更する場合は、`docs/designs/*.md` と `docs/designs/_llm/*.properties` を同一変更で同期更新する
- `_llm` だけ先に更新して完了扱いにしない
- design markdown だけ更新して `_llm` を放置しない
- `_llm` がまだ存在しない場合は、必要に応じて新規作成する
- 命名規則を追加または変更した場合は、`docs/designs/naming-rules.md` と `docs/designs/_llm/naming-rules.properties` を同一変更で同期更新する
- Post-Deploy Actual Values の共通定義を追加または変更した場合は、`docs/designs/post-deploy-actual-values.md` と `docs/designs/_llm/post-deploy-actual-values.properties` を同一変更で同期更新する
- 命名規則変更の影響があるサービス設計書、CloudFormation、テスト、結果記録も必要に応じて更新する
- CloudFormation 実装ルールまたは template 構成ルールを変更した場合は、影響を受ける設計書、CloudFormation、プロンプト、結果記録も必要に応じて更新する
- deploy 成功後は、必要な ID / 実値を `docs/designs/*.md` の `Post-Deploy Actual Values` に反映する
- テスト結果の証跡は `tests/results/*.md` に記録する

## 実装ルール

- 手動構築を主経路にしない
- CloudFormation を先に作成または更新し、その後 AWS CLI で実行する
- 既存環境変更時は、影響範囲、置換有無、依存関係を意識する
- 実装は design markdown と整合していること
- 実リソース名を持つ AWS リソースは naming rules に従う
- 実リソース名を持たない AWS リソースは `Name` tag に naming rules を適用する
- 命名規則との差分がある既存リソースを扱う場合は、rename の要否と影響を明示する
- CloudFormation template 構成は、`1 template = 1 deploy責務` に従う
- nested stack は使わない
- stack 間連携は cross-stack reference を前提とする
- template 分割の理由を設計書に残す
- template は、可能な限り運用に必要な重要 ID / 接続値を Outputs に出す
- Outputs に出していない ID は AWS CLI で取得して設計書へ反映する

## テストルール

- テストはシナリオベースで行う
- 正常系だけでなく、必要に応じて拒否系、失敗系、到達性確認も含める
- 疎通、利用シナリオ、権限挙動を優先して確認する
- 単なるパラメータ設定確認のみでは不十分とする
- 必要に応じて Resource Name または `Name` tag が期待どおりか確認する
- ただし、命名ルールの文字列一致だけでテスト完了としない
- テスト結果の証跡は `tests/results/*.md` に記録する
- `docs/` 配下にはテスト結果を置かない
- scenario script と result markdown は、原則として同じ basename にする
- 結果 markdown には、少なくとも以下を含める
  - テストステップ
  - 実行コマンド
  - 期待値
  - 実績値
  - pass / fail

## 記録ルール

最低限、以下を markdown に残すこと。

- 対象サービス
- 読んだ design markdown
- 読んだ `_llm` 補助ファイル
- 必要に応じて読んだ naming rules 関連ファイル
- 必要に応じて読んだ Post-Deploy Actual Values 関連ファイル
- 必要に応じて読んだ CloudFormation 実装ルールと template 構成ルール
- 更新した design markdown
- 更新した `_llm` 補助ファイル
- 変更した CloudFormation ファイル
- AWS CLI 実行コマンド
- 追加または更新したテスト
- 実施結果
- 未解決事項
- 次の対応候補

命名規則を追加または変更した場合は、必要に応じて以下も残すこと。

- 更新した命名規則ファイル
- 命名規則の正規化内容
- 既存命名との差分
- rename を見送る場合の理由
- 今後の移行方針

CloudFormation 実装ルールまたは template 構成ルールを変更した場合は、必要に応じて以下も残すこと。

- 更新した instruction ファイル
- template 境界の判断理由
- deploy責務の整理結果
- export / import の考え方
- 既存構成との差分
- review checkpoint
- phase
- last successful step
- next action

Post-Deploy Actual Values とテスト結果エビデンスについては、必要に応じて以下も残すこと。

- 反映した `Post-Deploy Actual Values`
- `tests/results/*.md` に作成または更新した証跡ファイル
- scenario script と result markdown の対応関係
- 期待値と実績値の差分
- pass / fail 判定

## 禁止事項

- 関連する `docs/designs/*.md` を読まずに実装を始めること
- `_llm` が存在するのに読まずに設定変更すること
- design markdown が古いまま CloudFormation を先に更新すること
- 設計書にない設定値を推測で採用すること
- `_llm` だけ更新して markdown を放置すること
- 手動変更を主経路にすること
- 設定値確認だけでテスト完了とすること
- 記録を残さずに完了とすること
- `docs/designs/naming-rules.md` を読まずに新規リソース名を決めること
- `docs/designs/_llm/naming-rules.properties` が存在するのに読まずに命名を決めること
- 実名と `Name` tag の適用先を曖昧なまま実装すること
- 命名規則との差分がある既存リソースを、影響評価なしに一括リネームすること
- `.github/instructions/cloudformation.instructions.md` を読まずに新規 stack 構成を決めること
- nested stack を前提に template 構成を決めること
- `1 template = 1 deploy責務` を満たさない構成を放置すること
- validate 成功後に人間レビューなしで deploy / update / execute-change-set に進むこと
- deploy 後の必要 ID / 実値を `docs/designs/*.md` に反映しないこと
- `docs/` 配下にテスト結果を置くこと
- テスト結果を CLI 出力の断片だけで済ませること

## 完了条件

以下をすべて満たした場合のみ完了とする。

- 関連する `docs/designs/*.md` を実装前に読んでいる
- 関連する `docs/designs/_llm/*.properties` が存在する場合は実装前に読んでいる
- 命名規則が関係する場合は `docs/designs/naming-rules.md` を読んでいる
- `docs/designs/_llm/naming-rules.properties` が存在する場合は読んでいる
- Post-Deploy Actual Values が関係する場合は `docs/designs/post-deploy-actual-values.md` を読んでいる
- `docs/designs/_llm/post-deploy-actual-values.properties` が存在する場合は読んでいる
- CloudFormation 実装または template 構成が関係する場合は `.github/instructions/cloudformation.instructions.md` を読んでいる
- 必要な design markdown が更新されている
- 必要な `_llm` 補助ファイルが更新されている
- 命名規則変更がある場合は markdown と `_llm` の両方が同期更新されている
- 実装が `infra/cloudformation/*` に反映されている
- `aws cloudformation validate-template` が成功している
- validate 成功後は `REVIEW_PENDING` で停止している
- 承認後のみ AWS CLI による deploy / update 方針または実行コマンドが使われている
- deploy 後の必要な `Post-Deploy Actual Values` が `docs/designs/*.md` に反映されている
- `tests/scenarios/*` にシナリオテストがある
- `tests/results/*.md` にテスト結果の証跡がある
- 証跡 markdown にテストステップ、実行コマンド、期待値、実績値、pass / fail が記録されている
- 実施結果が markdown に記録されている
