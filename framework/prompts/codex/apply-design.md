...
chatで確定した詳細設計をrepositoryへ保存し、generated LLM design mirrorを同期するために使用する。
...
- `Required changes`は一意なRequirement ID付きで、確定設計の保存とLLM mirror生成をfile単位に記載する。
- `Acceptance checks`は各Markdown／JSON／propertiesへ`changed:`または`exists:`を対応付ける。
- Allowed pathsは対象の`docs/designs/<environment>/<aws-account-id>/**`、`llm/designs/<environment>/<aws-account-id>/**`、`tasks/active.md`だけに限定する。
- IaC、actuals、scenario、scenario result、別targetは対象外とする。
...
`llm/designs/**`を手動編集しない。

## Generate LLM design mirror
